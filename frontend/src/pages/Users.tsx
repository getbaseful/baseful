...
export default function Users() {
  ...
  const handleSaveUserAccessAndPermissions = async () => {
    ...
    try {
      // SQLite write endpoints should be updated sequentially to avoid lock contention.
  ...
  const handleKickUser = async (targetUser: UserAccount) => {
    if (
      !confirm(
        `Kick ${targetUser.email}? This removes the account completely and they will be signed out on their next request.`,
      )
    ) {
      return;
    }
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <Button
                        ...
                        size="sm"
                        className="gap-1.5"
                        onClick={() => handleKickUser(account)}
